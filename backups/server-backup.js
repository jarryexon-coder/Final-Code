...
const USER_AUTH = {
  ...
  generateToken(userId) {
    return jwt.sign({ userId }, process.env.JWT_SECRET || 'test-secret-key-for-now', { expiresIn: '7d' });
  },
  
  verifyToken(token) {
    try {
      return jwt.verify(token, process.env.JWT_SECRET || 'test-secret-key-for-now');
    } catch (error) {
      return null;
    }
  },
